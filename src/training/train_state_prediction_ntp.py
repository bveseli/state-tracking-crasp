...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Directory containing per-task folders (e.g. lang_id_1, aa)",
    )
    parser.add_argument(
        "--save_path",
        type=str,
        default="results/main_language_suite/hyperparameter-search",
        help="Root directory for per-task sweep logs",
    )
    ...
    dataset_path = os.path.join(args.dataset_root, args.task)
    ...
    task_path = os.path.join(args.save_path, args.task)
    ...
    if max_n_layer >= 16:
        summary_prefix = "summary_large"
    elif max_n_layer > 4:
        summary_prefix = "summary_mid"
    else:
        summary_prefix = "summary_small"
    summary_f = open(os.path.join(task_path, f"{summary_prefix}.txt"), "w")
